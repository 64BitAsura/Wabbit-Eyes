 ...
 * via great-circle routes, mirroring the NMEA_Simulator-powered server.
 */
...
const MockClient = (() => {
  ...
  function generateGreatCircleWaypoints(origin, dest, numPoints) {
    ...
    if (d < 1e-10) return [{ lat: origin.lat, lng: origin.lng }];
  ...
  // --------------- NMEA-style sentence generation (mirrors NMEA_Simulator GPRMC) ---------------
  ...
  function toNMEALat(decimal) {
    const dir = decimal >= 0 ? 'N' : 'S';
    const abs = Math.abs(decimal);
    const deg = Math.floor(abs);
    const min = (abs - deg) * 60;
    return { val: String(deg).padStart(2, '0') + min.toFixed(4).padStart(7, '0'), dir };
  }

  function toNMEALon(decimal) {
    const dir = decimal >= 0 ? 'E' : 'W';
    const abs = Math.abs(decimal);
    const deg = Math.floor(abs);
    const min = (abs - deg) * 60;
    return { val: String(deg).padStart(3, '0') + min.toFixed(4).padStart(7, '0'), dir };
  }

  function generateGPRMC(lat, lon, speed, course) {
    const now = new Date();
    ...
    const latN = toNMEALat(lat);
    const lonN = toNMEALon(lon);
    const spdStr = speed > 0 ? speed.toFixed(1) : '';
    const crsStr = course > 0 ? course.toFixed(1) : '';
    const body = `GPRMC,${timeStr},A,${latN.val},${latN.dir},${lonN.val},${lonN.dir},${spdStr},${crsStr},${dateStr},,`;
    return `$${body}*${nmeaChecksum(body)}`;
  }

  function parseNMEACoord(str, dir) {
    if (!str) return 0;
    const isLat = (dir === 'N' || dir === 'S');
    const degLen = isLat ? 2 : 3;
    const deg = parseInt(str.substring(0, degLen), 10);
    const min = parseFloat(str.substring(degLen));
    let dec = deg + min / 60;
  ...
  function parseGPRMC(sentence) {
    const body = sentence.split('*')[0].replace(/^\$/, '');
    const f = body.split(',');
    return {
      lat: parseNMEACoord(f[3], f[4]),
      lon: parseNMEACoord(f[5], f[6]),
      speed: parseFloat(f[7]) || 0,
      course: parseFloat(f[8]) || 0,
    };
  }
  ...
  class Vessel {
    ...
    tick() {
      ...
      // Generate NMEA GPRMC sentence (mirrors NMEA_Simulator pipeline)
      const nmea = generateGPRMC(this.latitude, this.longitude, this.velocity, this.direction);
      const parsed = parseGPRMC(nmea);
      this.latitude = parsed.lat;
      this.longitude = parsed.lon;
    }
